...
d = read_csv("merged.abunds.tpm.types.csv")
metadata = read_csv("metadata.csv")

types = d %>% select(gene_symbol,class,subclass,type,subtype)
...
dl = dt %>% 
  pivot_longer(-Samples, names_to="gene_symbol", values_to="TPM")
...
dj = inner_join(dl, metadata, by="Samples") %>%
  inner_join(., types, by="gene_symbol") %>%
...
#   mutate(Category = ifelse(gene_symbol == "blaKPC", gene_symbol, Category)) %>%
#   mutate(Category = ifelse(gene_symbol == "blaVIM", gene_symbol, Category)) %>%
#   mutate(Category = ifelse(gene_symbol == "blaOXA-48_fam", gene_symbol, Category)) %>%
#   mutate(Category = ifelse(gene_symbol == "blaDIM-SIM-IMP", gene_symbol, Category)) %>%
#   mutate(Category = ifelse(gene_symbol == "blaNDM", gene_symbol, Category))
...
tpm = dj %>%
  ...
  mutate(total_TPM = sum(TPM)) %>%                  # sums TPM within a Category while keeping individual gene_symbol TPM
  ungroup() %>%
...
tf1 = tf %>%
  mutate(bc = factor(vector("character", length(tf$gene_symbol)))) %>%
  mutate(bc = if_else(gene_symbol == "blaKPC", "black", bc)) %>%
  mutate(bc = if_else(gene_symbol == "blaVIM", "black", bc))

# show labels and borders only for the most abundant genes
tf1 = tf1 %>%
  mutate(l = factor(vector("character", length(tf$gene_symbol)))) %>%
  mutate(l = if_else(TPM > 50, gene_symbol, l)) %>%
  mutate(bc = if_else(TPM > 50, "black", bc))

# count number of genes in each collection type
tf1 = tf1 %>%
  #count(gene_symbol, Collection_Type) %>%
...
p
ggsave("AMRbarplot.png", p)
ggsave("AMRbarplot.jpg", p)
  
# BETA-LACTAMASE only
# assign border colors to key genes
tf2 = tf %>%
  mutate(bc = factor(vector("character", length(tf$Category)))) %>%
  mutate(bc = if_else(Category == "blaKPC", "black", bc)) %>%
  mutate(bc = if_else(Category == "blaVIM", "black", bc))

# also show labels for the most abundant genes
tf2 = tf2 %>%
  mutate(l = factor(vector("character", length(tf2$gene_symbol)))) %>%
  mutate(l = if_else(TPM > 45, gene_symbol, l)) %>%
...
p2 = tf2 %>%
  ...
  coord_cartesian(clip = "off")
p2
ggsave("BLAbarplot.png", p2)
ggsave("BLAbarplot.jpg", p2)
